...
class _HomeState extends State<Home> {
  ...
  void initState() {
    ...
    Timer.periodic(Duration(seconds: 1), (timer) {
        if(!_isRunning){
          timer.cancel(); // 여기서 타이머 죽이기 때문에 dispose안써도됨
        }
        _addItem();
      },
    );
  ...
  String _chosenItem(DateTime now1){
    String chosenDateTime1 = '${now1.year}-'
                      '${now1.month.toString().padLeft(2,'0')}-'
                      '${now1.day.toString().padLeft(2,'0')}'
                      ' ${_weekDayToString(now1.weekday)}'
                      ' ${now1.hour.toString().padLeft(2,'0')}:'
